...
valoresDataFrame = data.frame(minutos = valores9,
                              alumnos = rep("alumnos que se duermen en clase", length(valores9))
                            )

ggplot(valoresDataFrame, aes(x = alumnos, y = minutos)) + 
  geom_boxplot() +
